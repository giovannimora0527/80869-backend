...
public class UsuarioServiceImpl implements UsuarioService {
    /**
     * Valida que los campos requeridos de UsuarioRq no sean nulos ni vacíos.
     * Lanza BadRequestException si algún campo obligatorio falta.
     */
    private void validarCampos(UsuarioRq usuarioNuevo) throws BadRequestException {
        if (usuarioNuevo == null) {
            throw new BadRequestException("El objeto usuario no puede ser nulo.");
        }
        ...
        if (usuarioNuevo.getEmail() == null || usuarioNuevo.getEmail().trim().isEmpty()) {
    ...
    public RespuestaRs guardarUsuario(UsuarioRq usuarioNuevo)
        ...
        String password = this.generarPass();
    ...
    @Override
    public RespuestaRs recuperarPassword(String username) throws MessagingException, BadRequestException {
        Optional<Usuario> optUser = this.usuarioRepository.findByUsername(username);
        RespuestaRs rta = new RespuestaRs();
        if (optUser.isEmpty()) {
            auditoriaRecuperacionService.registrarIntento(username, "Intento de recuperación fallido: usuario no existe");
            rta.setStatus(200);
            rta.setMensaje("Si el usuario existe, se enviará un correo con instrucciones");
            return rta;
        }
        Usuario usuario = optUser.get();
        String passwordTemporal = this.generarPass();
        ...
        auditoriaRecuperacionService.registrarIntento(username, "Recuperación exitosa: contraseña temporal enviada");
        rta.setStatus(200);
        rta.setMensaje("Si el usuario existe, se enviará un correo con instrucciones");
        return rta;
    }
